...
public class Test_MIE_UploadBrand extends APITestBase {
    String sHost;
    String sEndpoint;
    ...
    public void getEnvironment(ITestContext testContext) throws IOException {
        sHost = PropertiesHelper
                .getHostStringFromPropertiesFile(sPropertyFileName, Test_MIE_UploadBrand.class);
        sEndpoint = "/v2/utilities/Brands";
    }


    @Test(dataProvider = "dataProvider", dataProviderClass = TestDataProvider.class)
    public void uploadBrand(Map<Object, Object> testData, ITestContext testContext)
            throws IOException {
        ...
        String storeData = KeywordsHelper
                .replaceKeyword(testData.get("storedata").toString(), testContext);
        ...
        if (responseCode.startsWith("2") && !(storeData.trim().equals(""))) {
            DocumentContext documentContext = JsonPath.parse(sJSON);
            String[] validationNodes = storeData.split("~");
            for (int i = 0; i < validationNodes.length; i++) {
                String sValidationNode = validationNodes[i];
                String[] nodeValuePairs = sValidationNode.split(":");
                String sKey = nodeValuePairs[0];
                String sStoreContext = nodeValuePairs[1];
                Object sStoreDataValue = documentContext.read(sKey);
                setSuiteContextValue(sStoreContext, sStoreDataValue, testContext);
            }
        }
